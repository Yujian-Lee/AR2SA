...
def mask_iou(logits, target, eps=1e-7, size_average=True):
    r"""
        param:
            pred: size [N x H x W]
...
def Eval_Fmeasure(logits, gt, measure_path, pr_num=255, device='cuda:0'):
    r"""
        param:
            pred: size [N x H x W]
            gt: size [N x H x W]
    ...
    # convert N*C*H*W logits to N*H*W logits
    pred=2*logits[:,1]-logits[:,0]
...
def get_loss(loss,mask,task):
    if task=='v1m' and torch.mean(mask)==0:
